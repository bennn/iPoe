  ...
  (for/fold ([sym+word* sym+word*])
    ...
    (cond
     [(varmap-lookup sym+word* var)
      ...
      (if (rhyme=? rhyme l)
        (failure src (format "Line ~a of stanza ~a does not match rhyme scheme. Expected a word to rhyme with '~a' but got '~a'" (add1 line-num) (add1 n) rhyme l))
        (success src sym+word*)))]
     [else
      ;; Bind the current word to the variable
      (success src (varmap-add sym+word* var l))])))
