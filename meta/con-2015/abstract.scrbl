...
The rhyme scheme in this case specifies each line with a symbol rather than a pair.
Secretly, these lines elaborate into full pairs.
The first step of decoding a rhyme scheme will turn the first stanza @tt{[A B A B]} into the list of pairs @tt{[(A . *) (B . *) (A . *) (B . *)]}.
On the other hand, giving only a number will leave an asterisk (wildcard) for the rhyme.

Also note that we did not use the specification of couplets directly in our specification of sonnets.
...
Nothing more.
The language @tt{ipoe} then elaborates a specification into a new Racket language that implements the mechanical task of reading text from an input source and checking this new text against the rhyme scheme.
Additionally, elaborated programs include a pass to check spelling, and also support a variety of run-time flags to control details of the poem-checking process.
